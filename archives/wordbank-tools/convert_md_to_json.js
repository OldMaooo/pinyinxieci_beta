import { readFileSync, writeFileSync } from 'fs';
import { join } from 'path';

// Unit mapping from work plan
const unit_mapping = {
  "阅读1": 1, "阅读2": 2, "阅读3": 3, "语文园地一": 3,
  "识字1": 4, "识字2": 5, "识字3": 6, "识字4": 7, "语文园地二": 7,
  "阅读4": 8, "阅读5": 9, "阅读6": 10, "语文园地三": 10,
  "阅读7": 11, "阅读8": 12, "阅读9": 13, "阅读10": 14, "语文园地四": 14,
  "阅读11": 15, "阅读12": 16, "阅读13": 17, "语文园地五": 17,
  "阅读14": 18, "阅读15": 19, "阅读16": 20, "阅读17": 21, "语文园地六": 21,
  "阅读18": 22, "阅读19": 23, "阅读20": 24, "语文园地七": 24,
  "阅读21": 25, "阅读22": 26, "阅读23": 27, "语文园地八": 27
};

// Read MD file
const md_path = join(process.cwd(), 'AI_Data_Generation_Kit/intermediate_data/二年级上册.md');
const md_content = readFileSync(md_path, 'utf8');

// Find "## 三、最终总表" section
const finalTableSection = md_content.split('## 三、最终总表')[1];
if (!finalTableSection) {
  console.error('Could not find "## 三、最终总表" section');
  process.exit(1);
}

// Parse lessons from the section
const lines = finalTableSection.split('\n');
let current_lesson = null;
const lessons = {};

for (const line of lines) {
  if (line.startsWith('#### ')) {
    current_lesson = line.slice(5).trim();
    lessons[current_lesson] = [];
  } else if (line.trim() && !line.startsWith('#') && !line.startsWith('-') && !line.startsWith('|') && current_lesson) {
    // Extract words (comma-separated)
    const words = line.split(/、|，/).map(w => w.trim()).filter(w => w);
    lessons[current_lesson].push(...words);
  }
}

console.log('Parsed lessons:', Object.keys(lessons));

// Generate wordBank
const wordBank = [];
for (const [lessonName, words] of Object.entries(lessons)) {
  const unit = unit_mapping[lessonName];
  if (!unit) {
    console.warn(`No unit mapping for lesson: ${lessonName}`);
    continue;
  }
  for (const word of words) {
    wordBank.push({
      word,
      pinyin: "",
      grade: "二年级",
      semester: "上册",
      unit,
      lessonName
    });
  }
}

// Create JSON structure
const json = {
  version: "1.0",
  buildDate: new Date().toISOString(),
  gradeSemester: "二年级上册",
  count: wordBank.length,
  wordBank
};

// Write output
const output_path = join(process.cwd(), 'dist/data/二年级上册.json');
writeFileSync(output_path, JSON.stringify(json, null, 2), 'utf8');
console.log(`Generated JSON with ${wordBank.length} words at ${output_path}`);
