...
// Stop at the next "## " (double hash) which is "## 四、字数统计"
const [lessonContent] = finalTableSection.split(/## /).slice(1, 2);
if (!lessonContent) {
  console.error('Could not extract lesson content');
  process.exit(1);
}

// Split by "#### " to get individual lessons
const [beforeLessons, ...lessonParts] = lessonContent.split('#### ');
// First part contains everything before the first "####", skip it
...
for (const part of lessonParts) {
  ...
  if (!lessonName || lessonName === '处理规则' || lessonName === '最终总表') {
    continue;
  }
