...
// Extract from "### 最终总表" to next "## "
const parts = finalTableSection.split(/## /);
// The part before first "## " contains everything including "### 最终总表"
const beforeNextSection = parts[0];

// Now split on "### 最终总表" to get the actual lessons
const subsections = beforeNextSection.split('### 最终总表');
if (subsections.length < 2) {
  console.error('Could not find "### 最终总表" subsection');
  console.log('Subsections:', subsections.map((s, i) => `Part ${i}: ${s.substring(0, 100)}...`).join('\n'));
  process.exit(1);
}

const actualLessonContent = subsections[1];

// Split by "#### " to get individual lessons
const lessonParts = actualLessonContent.split('#### ').filter(p => p.trim());
...
for (const part of lessonParts) {
  ...
  const lessonName = firstLine.trim();
  
  // Skip if no lesson name
  if (!lessonName) {
    continue;
  }
